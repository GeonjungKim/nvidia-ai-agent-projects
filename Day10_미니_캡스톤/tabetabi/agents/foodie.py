...
8. 예산 상한을 반드시 지킨다: 저녁 후보는 search_restaurants에 max_dinner_budget, 점심 후보는 max_lunch_budget 인자를 넣어 검색한다 (계약에 값이 있으면). 弁当·惣菜·コロッケ 같은 테이크아웃 업태는 저녁에 넣지 마라. (코드가 예산 초과를 최종 제거하지만, 애초에 넣지 않는 게 후보 낭비를 막는다.)
