...
(4) 예산 상한이 있으면 저녁은 budget_dinner_floor ≤ max_dinner_budget, 점심은 budget_lunch_floor ≤ max_lunch_budget 인가 (코드도 이중으로 강제하지만 함께 확인한다)
external(사용자 지정 장소)은 조회 대상이 아니며, external=true인 locked 항목은 계약이 정상
반영된 것이다 — "locked인데 external이라 불일치" 같은 지적을 하지 마라.
...
async def run_critic(
    ...
    task = (
        ...
        f"저녁 예산 상한={contract.max_dinner_budget or '없음'}, 점심 예산 상한={contract.max_lunch_budget or '없음'}\n"
